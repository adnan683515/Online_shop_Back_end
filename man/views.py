...
class ShirtListView(APIView):
    ...
    def get(self,request,format=None):
        ...
        Brand_id = self.request.query_params.get('brand_id')
        if Brand_id:
            queryset = Product.objects.filter(Brand_shirt=Brand_id,type_your_product='Shirt')
...
####################  Watches  #############################
from .serializer import warenty_watches_serialzier,WatchSerializer,CetagorySerializer
from product_app.models import warenty_watch,MainMetariails
...
class WarentyView(APIView):
    ...
    def get(self,request,format=None):
        queyset = warenty_watch.objects.all()
        serializer = warenty_watches_serialzier(queyset,many=True)
        return Response(serializer.data)
...
class WatchProductView(APIView):
    ...
    def get(self,request,format=None):
        ...
        if warenty_id:
            queryset = Product.objects.filter(type_your_product='Watch',warenty_watch=warenty_id)
...
class ManJacketView(APIView):
    ...
    def get(self,request,format=None):
        ...
        if warrenty_id:
            queryset = Product.objects.filter(type_your_product="Jacket",warenty_watch=warrenty_id)
...
from .serializer import warenty_watches_serialzier

class JacksWarrentyView(APIView):
    
    def get(self,request,format=None):
        queryset = warenty_watch.objects.filter(choice_type='jacket')
        serializer = warenty_watches_serialzier(queryset,many=True)
        return Response(serializer.data)
